...
- The tool's own config lives next to the exe; its log lives in user data
  (`%LOCALAPPDATA%\reme-helper\log\reme-helper.log`, rotated at 1 MB with 3 backups). Memory data stays in ReMe's
  workspace, and **deleting reme-helper never deletes memories**.
...
- The `config.json` in a release is a factory template (containing nothing about the author's machine), generated by
  `scripts/make_release_config.py`. Your own `config.json` is never packaged into a release (it is gitignored).
